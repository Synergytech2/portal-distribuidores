...
const PRICE_FILE = process.env.PRICE_FILE || '';   // ruta en Dropbox, p.ej. /precios_distribuidor.json
...
async function getFeed(force = false) {
  ...
  let json;
  if (PRICE_FILE) {
    // Leer el archivo de precios directamente del Dropbox del propietario (con el token).
    if (!dropboxConfigured()) throw new Error('PRICE_FILE requiere DROPBOX_REFRESH_TOKEN');
    const { exists, content } = await downloadJson(PRICE_FILE);
    if (!exists || !content) throw new Error('No se encontró el archivo de precios en ' + PRICE_FILE);
    json = JSON.parse(content);
  } else if (PRICE_FEED_URL) {
    const res = await fetch(normalizeDropboxUrl(PRICE_FEED_URL), { cache: 'no-store' });
    if (!res.ok) throw new Error('Feed HTTP ' + res.status);
    json = await res.json();
  } else {
    throw new Error('Configura PRICE_FILE (ruta en Dropbox) o PRICE_FEED_URL (enlace)');
  }
